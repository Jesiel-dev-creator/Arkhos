...
export function PipelinePanel({ agents, currentAgent, totalCostEur }: PipelinePanelProps) {
  return (
    <div className="flex flex-col gap-1">
      ...
      {agents.map((agent, i) => {
        const isParallel = agent.name === "designer" || agent.name === "architect";
        const nextIsParallel = agents[i + 1]?.name === "architect";
          ...
          <div key={agent.name}>
            <div
              className={cn(
                ...
                agent.status === "running" && "bg-[var(--surface)]",
              )}
            >
              {/* Status icon */}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {isParallel && nextIsParallel && (
                    <span className="text-[10px] text-[var(--text-muted)] bg-[var(--elevated)] px-1.5 py-0.5 rounded">
                      parallel
                    </span>
                  )}
                </div>
