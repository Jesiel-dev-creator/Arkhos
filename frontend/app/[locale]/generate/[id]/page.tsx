...
export default function GenerationWorkspacePage() {
  ...
  // Auto-switch to chat when complete
  useEffect(() => {
    ...
    <div data-workspace className="h-screen flex flex-col overflow-hidden bg-[var(--void)]">
      <div className="flex-1 flex min-h-0">
        ...
        <div className="w-12 shrink-0 flex flex-col items-center py-3 gap-1 border-r border-[var(--border)] bg-[var(--deep)]">
          ...
          <ActivityButton icon={Eye} active={activeView === "preview"} onClick={() => setActiveView("preview")} label="Preview" />
          <ActivityButton icon={Code2} active={activeView === "code"} onClick={() => setActiveView("code")} label="Code" />
          <ActivityButton icon={FolderTree} active={activeView === "files"} onClick={() => setActiveView("files")} label="Files" />

          <div className="flex-1" />

          <ActivityButton icon={Workflow} active={bottomTab === "pipeline" && bottomExpanded} onClick={() => toggleBottom("pipeline")} label="Pipeline" badge={isWorking ? completedCount.toString() : undefined} />
          {isComplete && (
            <ActivityButton icon={MessageSquare} active={bottomTab === "chat" && bottomExpanded} onClick={() => toggleBottom("chat")} label="Chat" />
          )}

          <div className="mt-1">
            <Link href="/" aria-label="Back" className="flex items-center justify-center w-8 h-8 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-[var(--surface)] transition-colors duration-150 cursor-pointer focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:outline-none">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-3 px-4 py-2 border-b border-[var(--border)] bg-[var(--deep)]">
            <p className="text-xs font-[var(--font-code)] text-[var(--text-muted)] truncate max-w-[200px]">{generationId}</p>
            {state.profile && (
              <span className="text-[10px] font-medium uppercase tracking-wider text-[var(--text-muted)] px-2 py-0.5 rounded-md bg-[var(--surface)]">{state.profile}</span>
            )}
            <div className="flex-1" />
            {isComplete && (
              <a href={`${apiBase}/api/download/${generationId}`} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-[var(--brand)] text-white hover:brightness-110 focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:outline-none transition-all duration-150 cursor-pointer">
          ...
          {bottomTab && (
            <div className={cn("border-t border-[var(--border)] bg-[var(--deep)] flex flex-col", bottomExpanded ? "h-56" : "h-8")}>
              <div className="flex items-center gap-2 px-3 h-8 shrink-0 border-b border-[var(--border)]">
                <button type="button" onClick={() => { setBottomTab("pipeline"); setBottomExpanded(true); }} className={cn("text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded transition-colors cursor-pointer", bottomTab === "pipeline" ? "text-[var(--text-primary)] bg-[var(--surface)]" : "text-[var(--text-muted)] hover:text-[var(--text-secondary)]")}>
                  Pipeline {totalAgents > 0 && `(${completedCount}/${totalAgents})`}
                </button>
                {isComplete && (
                  <button type="button" onClick={() => { setBottomTab("chat"); setBottomExpanded(true); }} className={cn("text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded transition-colors cursor-pointer", bottomTab === "chat" ? "text-[var(--text-primary)] bg-[var(--surface)]" : "text-[var(--text-muted)] hover:text-[var(--text-secondary)]")}>
                    Iterate
                  </button>
                )}
                <div className="flex-1" />
                <button type="button" onClick={() => setBottomExpanded((p) => !p)} className="flex items-center justify-center w-5 h-5 rounded text-[var(--text-muted)] hover:text-[var(--text-secondary)] cursor-pointer focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:outline-none">
...
function ActivityButton({ icon: Icon, active, onClick, label, badge }: {
  icon: typeof Eye; active: boolean; onClick: () => void; label: string; badge?: string;
}) {
  return (
    <button type="button" onClick={onClick} aria-label={label} title={label} className={cn(
      ...
      active ? "bg-[var(--surface)] text-[var(--text-primary)]" : "text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-[var(--surface)]/50",
    )}>
      <Icon className="w-4 h-4" />
      {badge && (
        <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-[var(--brand)] text-white text-[8px] font-bold flex items-center justify-center">{badge}</span>
      )}
    </button>
  );
}
...
function HorizontalPipeline({ agents, currentAgent, status }: {
  ...
  const nodes = showSkeleton
    ? Array.from({ length: 5 }, (_, i) => ({ name: "", status: i === 0 ? ("running" as const) : ("pending" as const), model: undefined, costEur: undefined, durationS: undefined, isSkeleton: true }))
    ...
    <div className="h-full flex items-center px-6 gap-0 overflow-x-auto">
      {nodes.map((node, i) => (
        <div key={node.name || `skeleton-${i}`} className="flex items-center shrink-0">
          <div className={cn("flex flex-col items-center gap-1.5 px-4 py-3 rounded-xl transition-all duration-300 min-w-[90px]", node.status === "running" && "bg-[var(--brand)]/5")}>
            <div className={cn("w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300",
              ...
              {node.status === "complete" ? <Check className="w-3.5 h-3.5 text-[var(--success)]" /> :
               node.status === "running" ? <Loader2 className="w-3.5 h-3.5 text-[var(--brand)] animate-spin" /> :
               <Circle className="w-2.5 h-2.5 text-[var(--text-muted)]/30" />}
            </div>
            {node.isSkeleton ? <div className="h-3 w-14 rounded bg-[var(--surface)] animate-pulse" /> : (
              <span className={cn("text-xs font-medium capitalize",
                node.status === "running" ? "text-[var(--text-primary)]" : node.status === "complete" ? "text-[var(--text-secondary)]" : "text-[var(--text-muted)]/50",
              )}>{node.name}</span>
            )}
            {node.isSkeleton ? <div className="h-2 w-16 rounded bg-[var(--surface)]/50 animate-pulse" /> : (
              <span className="text-[9px] font-[var(--font-code)] text-[var(--text-muted)] truncate max-w-[80px]">
                {node.model ?? ""}{node.costEur !== undefined && node.costEur > 0 ? ` · €${node.costEur.toFixed(4)}` : ""}
              </span>
            )}
          </div>
          {i < nodes.length - 1 && (
    ...
    <div className="w-full h-full p-4 flex flex-col">
      <div className="rounded-xl border border-[var(--border)] bg-[var(--deep)] overflow-hidden flex-1 flex flex-col">
        ...
        <div className="px-6 py-3 border-t border-[var(--border)] flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-[var(--text-secondary)]">{agent ? t("workspace.agentWorking", { agent }) : t("workspace.starting")}</p>
            {total > 0 && <p className="text-[10px] text-[var(--text-muted)] tabular-nums">{completed}/{total} agents</p>}
          </div>
          <div className="flex-1" />
          {total > 0 && (
            <div className="w-24 h-1 rounded-full bg-[var(--surface)] overflow-hidden">
              <div className="h-full rounded-full bg-[var(--brand)] transition-all duration-700 ease-out" style={{ width: `${(completed / total) * 100}%` }} />
            </div>
          )}
        </div>
      </div>
