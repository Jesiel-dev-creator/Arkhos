    ...
    <div className="mx-auto max-w-5xl px-4 py-20">
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-3">
        {SECTION_KEYS.map((section) => (
          <section key={section} className="rounded-2xl border border-[var(--border)] bg-[var(--deep)] p-6">
            <p className="text-sm font-medium text-[var(--text-primary)]">{t(`sections.${section}.title`)}</p>
            <p className="mt-3 text-sm leading-relaxed text-[var(--text-secondary)]">{t(`sections.${section}.body`)}</p>
          </section>
        ))}
      </div>
    </div>
