...
const Collapse = ({
  ...
  return (
    <div
      className={`text-left border-y border-gray-200 dark:border-gray-700 overflow-hidden font-sans ${className || ""}`}
    >
      <h3
      ...
      >
        ...
        >
          ...
          >
            {title}
            <span
              className={`fill-current flex duration-200 ${
                _isOpen ? "rotate-180" : ""
              }`}
            >
              <ArrowIcon />
...
const CollapseGroup = ({ multiple = false, children }: CollapseGroupProps) => {
  ...
  const [openStates, setOpenStates] = useState(() =>
    collapses.map(
      (child) => (child.props as CollapseProps).defaultExpanded || false
    )
  );
